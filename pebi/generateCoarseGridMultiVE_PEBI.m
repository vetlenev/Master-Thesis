function CG = generateCoarseGridMultiVE_PEBI(G, VEGroups, colNo, varargin)
...
%}
    ...
    %[ii, jj, ~] = gridLogicalIndices(G); % Continue here
...
function [topz, bottomz, height, topcells] = getCellHeights(G, opt)
    faceNo = rldecode(1 : G.cells.num, ...
                    diff(G.cells.facePos), 2) .'; % map from half faces to associated cells
    ...
    if size(G.cells.faces, 2) > 1 && (any(strcmpi(G.type, 'cartGrid')) || any(strcmpi(G.type, 'processGRDECL')))
        ...
        hf_z = hf_pt(:, 3); 
    elseif size(G.cells.faces, 2) == 1 % 2D grid
        hf_z = hf_pt(:, 2); % second element of face centroid is z-value
        fno = G.cells.faces(:, 1);
        ftype = ones(size(fno));
    ...
    %assert(all(height >= 0))    
    if any(height < 0)
        warning('Found negative height of cells!')
    end
    height(height < 0) = 0;    
end
