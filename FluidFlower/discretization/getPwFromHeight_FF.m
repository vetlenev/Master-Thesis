function pW_local = getPwFromHeight_FF(pt, t, b, h, h_max, pW, g, rhow, rhog, swr, snr, varargin)
...
%}
    ...
    g = norm(g); % heights have been manipulated tobe positive, so use original norm of gravity
    
    dz_below = min(max((t-h_max) - b, 0), pt-b); % water below pt
    h_below = max(pt-(t-h), 0); % mobile CO2 below pt
    hmax_below = (pt-b) - h_below - dz_below; % residual CO2 below pt
