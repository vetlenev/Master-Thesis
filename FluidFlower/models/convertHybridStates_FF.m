function states = convertHybridStates_FF(model, model_fine, states_c, varargin)
...
function [state, sgMax, snMaxVE_bottom, vG_any] = convertState(model, model_fine, state_c, state0_c, sgMax_fine, ...
    ...
    [a_M, a_R, sg] = getGasSatFromHeightFine_FF(model, sG_c, T, t, B, b, h(p), h_T(p), h_B(p), swr, snr, ...
    ...
    pW = getPwFromHeight_FF(cz, t, b, h(p), h_T(p), pressure, g, rhow, rhog, swr, snr);
