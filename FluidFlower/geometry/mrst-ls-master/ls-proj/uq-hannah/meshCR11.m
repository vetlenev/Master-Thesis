...
pth = fullfile(mrstPath('ls-proj'), 'uq-hannah/');
...
meshpt  = load(fullfile(pth, 'meshCR11_datapoints_10m.mat'));
meshpt  = meshpt.stratiPoints;
% Resize to a square of 1m by 1m
meshpt.boundary = meshpt.boundary/2000;
for n=1:numel(meshpt.lines)
    meshpt.lines{n} = meshpt.lines{n}/2000; 
end

% plot
fig2D(), hold on
plot(meshpt.boundary(:,1), meshpt.boundary(:,2), 'k')
plotLinePath(meshpt.lines(3:end), 'b');
plotLinePath(meshpt.lines(1:2), 'r');
...
%if strcmp(resolution, 'coarse')
    cellSize = [10 10]; % m
    fcf      = 0.8;     % fault cells relative size to cellSize
...
%end
tic
G2D = compositePebiGrid2D(cellSize/2000, [1 1], ...
...
% Resize
G2D.nodes.coords = G2D.nodes.coords*2000;

% Make layered grid
nLayers = 1;                        % Number of layers
layerThickness = 1;                 % 1 m thick
...
%% Save data
if save_data
    save(fullfile(pth, ['meshCR11_cellSize' ...                      
                        num2str(cellSize(1)) 'm.mat']), ...
         'G', 'G2D', 'p', 'compartID', 'layerID')
end
