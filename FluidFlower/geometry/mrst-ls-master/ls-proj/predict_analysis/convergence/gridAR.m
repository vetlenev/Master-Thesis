...
%           {[FW], [HW]}
fname     = 'gridAR_5manualStrati_DT400-500';
name      = {'A', 'B', 'C', 'D', 'E'};
...
for k=1:Nstrat
    ...
    itnum = 0;
    while myFault.Disp/myFault.MatProps.thick < 400 || ...
          myFault.Disp/myFault.MatProps.thick > 500 % force thin faults
        if itnum == 0
...
if k == Nstrat
    ...
    ylabel('$k_{xx}^{\mathrm{ref.}} / k_{xx}$ [-]', latx{:}, 'fontSize', 12)
    xlim(limx)
...
if k == Nstrat
    ...
    ylabel('$k_{zz}^{\mathrm{ref.}} / k_{zz}$ [-]', latx{:}, 'fontSize', 12)
    xlim(limx)
    xticks([1 10 100 1000])
