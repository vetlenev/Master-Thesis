...
% Prepare loop
nStrat = numel(vcl);
nSim = [10 100 200 500 1000 2000 5000 10^4 10^5];
%nSim = [10 100 500 1000 2000];
...
maex = nan(nStrat, numel(nSim)-1); maez = maex;
ks2x = nan(nStrat, numel(nSim)-1); ks2z = ks2x;
for j = 1:nStrat
    p = [histcounts(log_k_md{j, end}(:, 1), edges, 'normalization', 'probability')', ...
         histcounts(log_k_md{j, end}(:, 3), edges, 'normalization', 'probability')'];
    idx = find(p(:,1)); idz = find(p(:,2)); % only count bins where p > 0
    for k = 1:(numel(nSim) - 1)
        % MAE
        pit = [histcounts(log_k_md{j, k}(:, 1), edges, 'normalization', 'probability')', ...
               histcounts(log_k_md{j, k}(:, 3), edges, 'normalization', 'probability')'];
        maex(j, k) = sum(abs(pit(idx, 1) - p(idx, 1))) / numel(idx);
        maez(j, k) = sum(abs(pit(idz, 2) - p(idz, 2))) / numel(idz);
        
        % KS2
        ks2x(j, k) = kstest2(log_k_md{j, k}(:, 1), log_k_md{j, end}(:, 1));
        ks2z(j, k) = kstest2(log_k_md{j, k}(:, 3), log_k_md{j, end}(:, 3));
    end
end
% Plot utils
sz = [14, 12];
...
for j=1:nStrat
    ...
    probs = [histcounts(log_k_md{j, end}(:, 1), edges); ...
             histcounts(log_k_md{j, end}(:, 2), edges); ...
             histcounts(log_k_md{j, end}(:, 3), edges)]' ./ nSim(end);
    limy = floor(log10(min(probs(probs > 0))));
    for k=1:numel(plotId)
        nexttile(tileIds(k, j))
        hold on
        histogram(log_k_md{j, plotId(k)}(:, 1), edges, 'Normalization', 'probability', ...
            'FaceColor', colrs(1, :), 'EdgeColor', colrs(1,:), ...
            'FaceAlpha', 1, 'DisplayName', '$k_{xx}$')
        histogram(log_k_md{j, plotId(k)}(:, 2), edges, 'Normalization', 'probability', ...
            'FaceColor', colrs(2, :), 'EdgeColor', 'none', ...
            'FaceAlpha', .7, 'DisplayName', '$k_{yy}$')
        histogram(log_k_md{j, plotId(k)}(:, 3), edges, 'Normalization', 'probability', ...
