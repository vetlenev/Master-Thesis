...
meshpt  = load(fullfile(pth, 'gom_datapoints_noAmpDiv.mat'));
meshpt  = meshpt.s;

% Rescale to 1 for meshing
l = max(meshpt.boundary(:,1));
meshpt.boundary = meshpt.boundary/l;
meshpt.wells{1} = meshpt.wells{1}/l;
meshpt.wells{2} = meshpt.wells{2}/l;
meshpt.wells{3} = meshpt.wells{3}/l;
...
plotLinePath(meshpt.lines([1:2 9]), 'r');
plotLinePath(meshpt.lines(19), 'b');
plotLinePath(meshpt.wells(1:3), '.r', 'markerSize', 10);
...
G2D = compositePebiGrid2D(cellSize,          [L(1) L(2)], ...
                          'polybdr',         meshpt.boundary, ...
                          'faceConstraints', meshpt.lines([1:2 9 19]), ...
                          'FCFactor',        0.1, ...
                          'interpolateFC'  , false);                      
        
G2D = compositePebiGrid2D(cellSize, [L(1) L(2)], ...
            'polybdr', meshpt.boundary, ...          % CellSize & Outline
            'faceConstraints', meshpt.lines, ...     % Fault lines + contacts
            ...
            'interpolateCC'  , false, ...            % interpolate along well cells
            'CCFactor'       , 0.1 ,  ...            % Relative size of well cells
            'mlqtMaxLevel', 3);                      % nr of refinement steps around well
...
plotFaces(G2D, logical(G2D.faces.tag), 'edgeColor', 'b', 'lineWidth', 2); % Lines
