function perm = myUpscalePermDim(g, cg, rock, varargin)
% LS: commented out current lines 75-79 to avoid "no module mapping found 
%     for" warning in parfor
%
% LS: This function allows choosing a single dimension for the upscaling
% (e.g. grid may be 3d, but we only want upscaling in z direction). The
% method is only tpfa. Based on myUpscalePerm (MRST)
%
...
%    end
   ...
   assert(~isempty(opt.dim), 'You must pass a value for dim!')
   ...
   ups = @(cells, hf) upscale_block_tpfa(...
       extractSubgrid(g, cells), T(hf), opt.dim, fluid, opt.LinSolve);
...
function k = upscale_block_tpfa(subG, subT, dim, fluid, linsolve)
%
...
%           kx* = (q1*L)/(area(r_boundary)*(p2 - p1))
   ...
   L    = abs(subG.faces.centroids(faces1(1), dim) - ...
       subG.faces.centroids(faces2(1), dim));
