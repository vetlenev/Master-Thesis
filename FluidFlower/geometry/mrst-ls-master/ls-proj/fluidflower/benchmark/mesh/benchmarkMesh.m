...
%meshpt  = load(fullfile(pth, 'benchmark_datapoints.mat'));
...
plotLinePath(meshpt.lines(17:end), 'b');
plotLinePath(meshpt.lines(1:16), 'r');
plotLinePath(meshpt.wells(1:2), '.r', 'markerSize', 10);
plotLinePath(meshpt.wells(3:4), '.c', 'markerSize', 10);
...
if h==20
    G2D = compositePebiGrid2D(cellSize, [L(1) L(2)], ...
                             ...
                             'FCFactor'       , 0.25            , ... % Relative size of fault cells
...
elseif h == 10
    G2D = compositePebiGrid2D(cellSize, [L(1) L(2)], ...
                             ...
                             'interpolateCC'  , false, ...
                             'CCFactor'       , 0.5 ,  ...           % Relative size of well cells
                             'interpolateFC'  , true);             % Interpolate along faul
elseif h == 8
    G2D = compositePebiGrid2D(cellSize, [L(1) L(2)], ...
                             'polybdr', meshpt.boundary, ...       % CellSize & Outline
                             'faceConstraints', meshpt.lines, ...  % Fault lines
                             ...
                             'CCFactor'       , 0.5 ,  ...         % Relative size of well cells
                             'interpolateFC'  , true);             % Interpolate along fault lines
elseif h == 6.5     % 6mm does not work (weird mesh) for some reason
    G2D = compositePebiGrid2D(cellSize, [L(1) L(2)], ...
        'polybdr', meshpt.boundary, ...       % CellSize & Outline
        'faceConstraints', meshpt.lines, ...  % Fault lines
        'FCFactor'       , 0.75           , ...  % Relative size of fault cells
        'cellConstraints', meshpt.wells , ... % Well coordinates
        'interpolateCC'  , false, ...
        'CCFactor'       , 0.75 ,  ...        % Relative size of well cells
        'interpolateFC'  , true);             % Interpolate along fault lines
elseif h == 5 || h == 5.5
    G2D = compositePebiGrid2D(cellSize, [L(1) L(2)], ...
        'polybdr', meshpt.boundary, ...       % CellSize & Outline
