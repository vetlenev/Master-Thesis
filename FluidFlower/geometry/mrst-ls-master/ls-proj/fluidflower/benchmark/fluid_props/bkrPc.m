...
%
clear, close all
model_case = 22;        % 1 medium match, 2 reported, 22 reported modified
sg_pce = 1e-3;  
pcmult = 1.5;
sandtype = 'D';
...
elseif model_case == 2 || model_case == 22 % data in description.pdf
...
if model_case == 1
    ...
    swc = round(0.01*(3.5*((n*1e2)^1.26)/((k*1e3)^0.35)-1), 2); % in [0 1]
elseif model_case == 2 % data in description.pdf
    ...
    pce = pcev(ids);
elseif model_case == 22 % modif krel gas and krel wat from data in description.pdf
...
if model_case == 1
    ...
    pc_fit_scaled = pcmult*pc_fit_scaled.*sqrt((data.pc_k*n)./(data.pc_n*k));
elseif model_case == 2
...
elseif model_case == 2 || model_case == 22
    sgt_max = sgt;
