...
d_mm = [0.2, 0.66, 1.45, 1.77];  % ESF, C, E, F [mm]
porov = [.37, .38, .39, .39];
...
sw(end-1) = 1-sgv1;         % Has to be > 0.999 otherwise very low sat may enter the seal.
ns = (sw-swc) ./ (1-swc);
nspcmin = 0.01;
...
pc_fit_scaled = pc_mult*pc_fit_scaled.*sqrt((data.pc_k*n)./(data.pc_n*k));
...
l1 = plot(sw, krg_fit_scaled, '-r', 'linewidth', 2, ...
         'DisplayName', '$k_{r\mathrm{g}}$: $0.97(1-S_w^*)^{1.39}$, Swc');
hold on
l2 = plot(sw, krw_fit_scaled, '-b', 'linewidth', 2, ...
         'DisplayName', '$k_{r\mathrm{w}}$: $(S_w^*)^{4.192}$, Swc');
xlabel('$S_\mathrm{w}$ [-]', latx{:}, 'fontSize', 14)
ylabel('$k_r$ [-]', latx{:}, 'fontSize', 14)
%set(gca, 'yscale', 'log')
ylim([0 1]), yticks(0:.1:1)
