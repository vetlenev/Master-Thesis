...
bctype = 'none';                                                % constant pressure (cp) or pore volume multiplier (pvm)
depth = '1km'; % 'surface' or '1km'


%% Save data
folderName = ['co2brine_test_' depth];
...
y = [0.02 0.08];
G = computeGeometry(G);
idCO2 = all([G.cells.centroids(:, 3) > z, G.cells.centroids(:,2) > y(1), ...
             G.cells.centroids(:,2) < y(2)], 2);

% Plot
% fig3D(); plotGrid(G); plotGrid(G, idCO2, 'edgecolor', 'r'); 
% view([90, 0])
...
if strcmp(depth, 'surface')
    pth = fullfile(mrstPath('ls-proj'), 'fluidflower/medium/test/co2brine_test_props');
    fn  = fullfile(pth, 'co2brine_surf.DATA');
else
...
p_r = valp + g*rho_wr*water_column; % p at shallowest z
...
s0(idCO2, :) = repmat([0.3092, 0.6908], [sum(idCO2), 1]);
rs0 = zeros(G.cells.num, 1);
rv0 = 0;             
state0 = struct('s', s0, 'rs', rs0, 'rv', rv0, 'pressure', p0);

% %plot
% fig3D(); plotCellData(G, s0(:,2), 'edgealpha', 0.2)
...
%% No Wells        
simtime = 1*day;
reportTimes = [[5:5:60 70:10:120 140:20:180]*second [4:9 10:10:24*60]*minute];                             
...
plotGrid(G, idCO2, 'edgecolor', [0.5 0.5 0.5]);
