...
%            prp.(kw) = feval(['complete', kw], prp.(kw));
         ...
         case 'EHYSTR'  % LS >
             nrec = 1;                       % this keyword has only 1 record independent of the number of imb regions
             tmpl = {0.1, 2, 1.0, 0.1, 'BOTH', 'RETR', 'DRAIN', 'OIL', ...
                     'NO', 'NO', 'NO', 0.0, 0};     % default options in ECLIPSE
             data = readDefaultedKW(fid, tmpl, 'NRec', nrec);
             prp.(kw) = data;  
             prp.(kw)(1:4) = num2cell(str2double(data(1:4)));                     
             clear tmpl nrec         % LS <
