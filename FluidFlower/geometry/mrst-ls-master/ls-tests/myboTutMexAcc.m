...
%% Run a parallel simulation with 16 threads
maxNumCompThreads(16)
solver.amgcl_setup.nthreads = 16; % Specify threads manually
[ws1, states1, rep1] = simulateScheduleAD(state0, model, schedule, 'NonLinearSolver', nls);
...
total1 = sum(vertcat(par1.Total));
assembly1 = sum(vertcat(par1.Assembly));
lsolve1 = sum(vertcat(par1.LinearSolve));
lsolve_prep1 = sum(vertcat(par1.LinearSolvePrep));
its1 = rep1.Iterations;

a = [total0, assembly0, lsolve0, lsolve_prep0];
b = [total, assembly, lsolve, lsolve_prep];
c = [total1, assembly1, lsolve1, lsolve_prep1];

a(end+1) = a(1) - sum(a(2:end));
b(end+1) = b(1) - sum(b(2:end));
c(end+1) = c(1) - sum(c(2:end));

figure;
bar([a; b; c])
legend('Total time', 'Assembly', 'Linear solve', 'Linear solve preparation', 'Other functions')
set(gca, 'XTickLabel', {'Serial', '4 threads' [num2str(N), ' threads']})
%% Other notes
