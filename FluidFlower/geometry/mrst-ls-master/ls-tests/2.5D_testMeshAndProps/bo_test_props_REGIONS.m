...
rock.regions.saturation = ones(G.cells.num,1); rock.regions.saturation(G.cells.num/2+1:G.cells.num) = 2;
...
fn = 'C:\Users\lsalo\matlab\mrst-2019a\myprojects\2.5Dtestmesh\eclipse_data_files\ftest_REGIONS.DATA';
deck = convertDeckUnits(readEclipseDeck(fn));
% Indicate regions for both fluid and rock
satn = ones(G.cells.num,1); satn(id_seal)=2;
rockn = ones(G.cells.num,1); rockn(id_seal)=2; % can it be different from satn? 
...
% The warning appears in assignSGOF.m called by initDeckADIFluid.m when
...
show(cellInx) = false;
plotCellData(G,p0/barsa, show,'EdgeColor','k', 'FaceAlpha', 0.5);
...
%% Boundary conditions
f = boundaryFaces(G);        
fp = f(G.faces.centroids(f,3) < max(G.faces.centroids(f,3))-10);
...
for n=1:numel(states)
    states{n}.dp = (states{n}.pressure - states{1}.pressure)/barsa;  
end
