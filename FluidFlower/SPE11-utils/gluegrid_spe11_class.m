...
%% Plot topmost grid with defining points
% figure()
% plotGrid(Gtop)
% hold on
% plot(ptop_bottom(:,1), ptop_bottom(:,2), 'r.', 'markersize', 20)
% hold on
% plot(ptop_top(:,1), ptop_top(:,2), 'b.', 'markersize', 20)
...
function [poly_obj, nodes_overlap, pts_overlap] = glueToUpperPolygon(all_polys, poly_obj, poly_num, poly_num_upper, ...
    ...
    poly.top_side = top_side;
    poly.bottom_side = bottom_side;    
...
function [poly_obj, nodes_overlap, pts_overlap] = gluePinchOut(all_polys, poly_obj, poly_num, poly_num_upper, poly_num_pinch, ...
    ...
    %polyA.bottom_mask = bottom_mask;
    %polyA.top_mask = top_mask;
    ...
    %polyB.bottom_mask = bottom_mask;
    %polyB.top_mask = top_mask;
