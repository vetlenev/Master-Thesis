...
def fugacityCoefficientCO2(T, p, rhoCO2):
    ...
    p_bar = p/1e5 # gas phase pressure in bar
...
parser.add_argument(
    "-p1", "--min_press", required=True, type=float, help="The minimum gas pressure in Pascal."
)
parser.add_argument(
    "-p2", "--max_press", required=True, type=float, help="The maximum gas pressure in Pascal."
)
parser.add_argument(
    "-np",
    "--n_press",
...
outFile.write(f"# min gas pressure = {pressures.min}, max gas pressure = {pressures.max}, #pressure sampling points = {pressures.numSamples}\n#\n")
outFile.write("# temperature [K], gas pressure [Pa],         y_H2O [-],         x_CO2 [-]\n")
...
for i in range(temperatures.numSamples):
    ...
    for j in range(pressures.numSamples):
        ...
        A = computeA(T, p, rhoCO2); # (11)
        B = computeB(T, p, rhoCO2); # (12)
        y_H2O = (1 - B)/(1/A - B) # (13)
        x_CO2 = B*(1 - y_H2O) # (14)
