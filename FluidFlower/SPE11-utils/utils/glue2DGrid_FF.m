function G = glue2DGrid_FF(G1, G2, varargin)
%Connect two 2D grids along common edges.
% Currently specific for FluidFlower grid, 
% to handle collapsing cells between layers.
%
% SYNOPSIS:
%    G = glue2DGrid(G1, G2)
...
%}
   ...
   opt = struct('tol', 1.0e-3, 'poly_idx', []);
   ...
   mergefaces = identify_mergefaces(loop1, loop2, cnodes, G1_conformal.faces.num, opt.poly_idx);
...
% re-number the remaining faces to avoid gaps in numbering.
   ...
   missing_faces = setdiff(1:G.faces.num, G.cells.faces(:,1));
   ...
   % Update G.cells
   N = max([missing_faces(:); G.cells.faces(:,1)]);
   reindex = (1:N)';
   reindex(missing_faces) = [];
   new_ixs = nan(N,1);
   new_ixs(reindex) = 1:numel(reindex);
   Gcf_old = G.cells.faces(:,1);
   G.cells.faces(:,1) = new_ixs(Gcf_old);
   %Gcf_new = G.cells.faces(:,1);
   %G.cells.faces(:,2) = G.cells.faces(G.cells.faces(:,1), 2); 
end
...
function mergefaces = identify_mergefaces(loop1, loop2, cnodes, offset, varargin)
...
% the 'cnodes' table).
   ...
   % --- MODIFIED ---
   %cnodes = unique(cnodes, 'rows', 'stable');
   % ----------------
