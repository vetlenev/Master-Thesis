function [Gt, G, transMult] = topSurfaceGrid_FF(G)
...
function [Gt, orient] = construct_grid_from_top_faces(G, tcells, tfaces)
   ...
   Gt.nodes = struct('num', size(unodes, 1), 'coords', unodes(:,1), 'z', unodes(:,2));
   ...
   Gt.cells = struct('num', numel(tfaces), 'faces', faces, ...
                     'facePos', [1; last_node_ix+1], 'z', G.faces.centroids(tfaces, 2));
...
% size.  It does not take into account whether cells are active or not.
   ...
   elseif isfield(G, 'cartDims')
      ...
      lnumber = G.cartDims(3);
       ...
       lsize = max(G.i) - min(G.i) + 1;
       lnumber = max(G.j) - min(G.j) + 1;
   end
end
