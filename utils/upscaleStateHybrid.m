function state = upscaleStateHybrid(coarsemodel, model, state)
...
%}
    ...
    state.s = s;
    state.sGmax = s(:,2); % assume gas phase is given by second element!
    ...
    if isprop(model, 'disgas') && model.disgas && isfield(state, 'rs')
        ph = model.getActivePhases; % [aquous, oil, gas]
        if ph(2)
            sX_ph = 'sO'; % oil phase present
        else
            sX_ph = 'sW'; % otherwise dissolve into aquous phase
        end
        sX = model.getProp(state_f, sX_ph);
        sX_c = coarsemodel.getProp(state, sX_ph);
        state.rs = accumarray(p, sX.*state.rs.*pvf)./(sX_c.*pvc);
