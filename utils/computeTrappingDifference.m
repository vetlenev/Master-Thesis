function [max_diff, max_diff_year, max_CO2, ...
            sum_diff, sum_CO2] = computeTrappingDifference(ax, report_hybrid, report_fine, varargin)
% Compute difference in trap categories between hybrid and fine model, and
% plot stacked difference over time.
% NB: Categories 'Residual' and 'Residual in plume' are merged!
% 
% The simulation result (set of states) first needs to be repackaged using the
% 'makeHybridReports' and 'makeFineReports' function.
%
% SYNOPSIS:
...
%}
    ...
    mass_fine(:,3) = mass_fine(:,3) + mass_fine(:,4);
    mass_fine(:,4) = [];
    mass_fine = convertTo(mass_fine, mega*kilo);
    mass_hybrid(:,3) = mass_hybrid(:,3) + mass_hybrid(:,4);
    mass_hybrid(:,4) = [];
    mass_hybrid = convertTo(mass_hybrid, mega*kilo);

    mass_diff = abs(mass_fine - mass_hybrid);  
    ...
    % Structural plume
    trap_category = {'dis', 'struct_res', 'res', 'struct_sub', 'struct_mob', 'free_mob', 'exit'};
    ...
    for i = 1:numel(chld)
        ...
        if strcmpi(obj.Type, 'hggroup') || strcmpi(obj.Type, 'area')
            ...
            elseif (cur_col == 7) && skip_dissolution
                set(get(get(chld(i), 'Annotation'), 'LegendInformation'), ...
                    'IconDisplayStyle', 'off');
            end
            cur_col = cur_col + 1;
