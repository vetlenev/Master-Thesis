...
%}
    ...
    for idx=1:2
        ...
        veH{idx} = find(T ~= t); % all virtual ve cells except top one (this is for top segregated plume)
    ...
    if ~isfield(state0, 'h_T') || ~isfield(state0, 'h_B') || ~isfield(state0, 'h')
       state0.h_T = zeros(size(state0.s(:,2))); 
       state0.h_B = zeros(size(state0.h_T));
       state0.h = zeros(size(state0.h_T));
    end
          
    [vW, vG, mobW, mobG, upcw, upcg, ...
        h, h_max, h_T, h_B, cellsBH, hHi_state, hBHi_state] = computeHybridFluxesRVE(model, pW, sG, muW, muG, rhoW, rhoG, trans, sgMax, state0.vGsum, state0.vGsum_sMax, cB, veB, cH, veH);
            
    state.h = h;
    state.h_max = h_max;
    state.h_T = h_T;
    state.h_B = h_B;
    % Only store RVE cells in a single state to avoid redundancy
    ...
    % only update vGsum_sMax if new sMax reached this time step
