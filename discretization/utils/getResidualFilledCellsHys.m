function [c_VE_Not, c_VE] = getResidualFilledCellsHys(model, Sn, vG)
    ...
    for idx=1:2 % CONSIDER REMOVING LOOP: veBottomFilled and veTopEmpty will never both occur as it stand now!
        ...
        if any(isVE_c) % reconstruct specifically for ve col transitioning to a lower layer
            ...
            Sn_c = value(Sn(c));                               
            veBottomFilled = (~model.fluid.hys).*((B == b & T ~= t) & (vG_veTrans > 0 & Sn_c < snr)); % bottom of col is filled but co2 hasn't reached top yet => not in VE!            
            veValid = (B == b & T ~= t) & (vG_veTrans > 0) ...
                                        & (~model.fluid.hys).*(Sn_c >= snr); % residual plume has reached top -> from prf to frf -> VE assumption holds
