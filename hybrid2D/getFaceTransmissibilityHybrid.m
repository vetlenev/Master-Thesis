function T = getFaceTransmissibilityHybrid(G, rock, fineToVE, varargin)
...
%}
   ...
   multargname = '';
   if nargin > 3
       multargname = inputname(4);
   end      
   [mult, varargin] = get_multipliers(multargname, G, rock, varargin{:});

   T = compute_onesided_trans(G, rock, fineToVE, varargin{:});
...
function T = compute_onesided_trans(G, rock, fineToVe, varargin)
   if has_cpgeometry(G)
      cpgeom = G.cells.cpgeometry;

      T = computeTransHybrid(G, rock, fineToVE, varargin{:}, ...
                       'K_system'       , 'loc_xyz', ...
