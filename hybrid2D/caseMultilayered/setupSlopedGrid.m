...
if standard
    ...
    G.nodes.coords(:,3) = G.nodes.coords(:,3) + m*G.nodes.coords(:,1) ...
                        + (nz/2.5)*sin(pi+8*pi*G.nodes.coords(:,1)/max(G.nodes.coords(:,1))) ...
                        + (exp(-G.nodes.coords(:,1)/max(G.nodes.coords(:,1))) * ...
                                    (nz/2.5)) .* cos(pi+4*pi*G.nodes.coords(:,1)/max(G.nodes.coords(:,1))) ...
                        + (nz/4)*sin(pi+20*pi*G.nodes.coords(:,1)/max(G.nodes.coords(:,1))); % linear slope of aquifer
end
