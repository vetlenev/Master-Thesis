function [state0, models, schedule, ...
            isFine, setZeroTrans, allSealingFaces] = setupStochasticGrid(faceConstraint, adaptiveSealing, dims, sizes, varargin)
...
if adaptiveSealing % Assign faces or cells based on thickness   
    ...
    for i=1:size(k_pos,1)        
        ...
        if abs(z_range(1) - z_range(2)) < dz_eps
            ...
            perm(sealingCells) = sealingCellsPerm(pk); % assign a random low-perm value ?
            pk = pk + 1;
            if pk > numel(sealingCellsPerm)
                pk = 1;
            end
        end        
    end
