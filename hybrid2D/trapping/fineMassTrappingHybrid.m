function [masses, masses_0] = fineMassTrappingHybrid(Gt, Gh, cFine, cmap, p, sG, sW, h, h_max, ...
...
%}
    ...
    SW = sW .* Gh.cells.height; % used to check that all elements in trapping inventory sum up to net fluid amount
    SG = sG .* Gh.cells.topDepth; % height of mobile plume
    rs = opt.rs;    
    ...
    pv       = rock.poro(cfine) .* Gt.cells.volumes .* pvMult; % think Gt.cells.volumes is fine -> it's just the surface area of each top surface cell
    if isfield(rock,'ntg')
       pv = rock.poro(cfine) .* Gt.cells.volumes .* rock.ntg .* pvMult;
