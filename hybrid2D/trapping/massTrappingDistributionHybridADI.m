...
%}
    ...
    [~, ~, kk] = gridLogicalIndices(Gh.parent);
      ...
      cf_ve = c_ve(ismember(c_ve, kkf));         
      ch_ve = ph(cf_ve); % only top fine cells needed -> should be same size as traps_ve       
      ch_fine = ph(c_fine); % parent selection for ALL cells in fine region
...
function [masses, masses_0] = massTrappingDistributionFine(Gt, Gh, c_fine, p, sG, sW, ...
    ...
    if isfield(traps, 'z_spill_loc')
        % trap heights -> only calculated from geometry, not h or h_max
        zt = max(traps.z_spill_loc(ctraps_fine) - top_z, 0); % Gt.cells.z --> ct_hybrid_z
        zt = min(zt, tot_height); % Gt.cells.H --> ct_hybrid_height
    else
        tr   = traps.trap_regions(ctraps_fine);
        tr_u = setdiff(unique(tr), 0);
        tr_z = [min(top_z); traps.trap_z(tr_u)]; % trap region zero -> spills out of domain -> set z-value of spill point to highest point in grid
        tr   = 1 + tr;
        tr(tr>numel(tr_z)) = 1; % to ensure all trap regions with no calculated heights are assigned zero spill region height
