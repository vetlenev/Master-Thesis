...
for i=1:numel(isFineCells.sealingBottom)
    ...
    [Gs, cmap, fmap, nmap] = UtilFunctions.extractLayerSubgrid(G, subfaces); 
    Gsi = cat(1, Gsi, Gs); % subset of original grid
    cmapi = cat(1, cmapi, cmap);
    fmapi = cat(1, fmapi, fmap);
    nmapi = cat(1, nmapi, nmap);
    
    Gt = topSurfaceGrid(Gs); % top surface of subgrid
...
%% Find traps
Gt = topSurfaceGrid(G); % grid from fine model
ta = trapAnalysis(Gt, true);
...
hybrid_reports = makeHybridReports(Gt, Gti, Gh, cmapi, fmapi, nmapi, {state0_hybrid, states_hybrid{:}}, ...
                                    mh.rock, mh.fluid, schedule_hybrid, [swr, snr], ta, tai, []);
