...
pe_sealing = convertTo(2*10^4*Pascal, barsa); % higher entry pressure in aquitards
pe_rest = 0*barsa;
p_cap = 5*pe_sealing;
...
if useFaceConstraint
    plot_dir = strcat(rootdir, '../Master-Thesis/book_illustrations/hybrid2D/sloped/figs/face_pc_bottom/');   
else
    plot_dir = strcat(rootdir, '../Master-Thesis/book_illustrations/hybrid2D/sloped/figs/cell_pc_bottom/');
end
mkdir(plot_dir);
...
figure(5)
vtc = model_hybrid.operators.connections.veInternalConn;
