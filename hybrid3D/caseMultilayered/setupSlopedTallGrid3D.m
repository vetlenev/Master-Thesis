function [state0, models, schedule, isFine, setZeroTrans, allSealingFaces] = ...
            setupSlopedTallGrid3D(faceConstraint, adaptiveSealing, trans_multiplier, dims, sizes)
...
elseif nargin < 4
    dims = [20, 10, 75]; % 200, 1, 50    
    sizes = [150, 150, 800]; % 800, 1, 250    
    if nargin < 3
        trans_multiplier = 1;
        adaptiveSealing = false;
        faceConstraint = true;
    end
end
...
z_pos = [lz/6, lz/5; ... % 0, nz/10
         lz/3, lz/2.8; ... % nz/5, nz/4
         lz/2, lz/1.9; ... % nz/3, nz/2.5
          2*lz/3, 2*lz/2.9];
          %kbc(1), kbc(2)];
...
G.nodes.coords(:,3) = G.nodes.coords(:,3) + ...
                        + (lz/50)*sin(2*pi*G.nodes.coords(:,1)/(0.4*max(G.nodes.coords(:,1)))) ...<
                        + (exp(-3*G.nodes.coords(:,1)/max(G.nodes.coords(:,1))) * ...
                                    (lz/20)) .* cos(pi/2+6*pi*G.nodes.coords(:,1)/max(G.nodes.coords(:,1))) ...
                        + (lz/40).*sin(pi/3+2*pi*G.nodes.coords(:,2)/(0.4*max(G.nodes.coords(:,2)))); % (nz/5) (pi/3 + 2*pi)                                                        
...
bc = [];
bc = pside(bc, G, 'Top', 100*barsa, 'sat', [1, 0]);
bc.value = bc.value + fluid.rhoWS.*G.faces.centroids(bc.face, 3)*norm(gravity); % since grid is skewed, top cells are at different depths and will have different pressure values
bc_f = bc.face;
bc_c = G.faces.neighbors(bc_f,:);
bc_c = bc_c(bc_c ~= 0);
min_kbc = min(kk(bc_c));
max_kbc = max(kk(bc_c));
kbc = [min_kbc, max_kbc+(min_kbc == max_kbc)];
...
if adaptiveSealing % Assign faces or cells based on thickness
    dz_eps = 10; % thickness threshold
    
    for i=1:size(k_pos,1)        
        z_range = [z_pos(i,1), z_pos(i,2)]; 

        if abs(z_range(1) - z_range(2)) < dz_eps % assign sealing face
            sealing_type = 'faces';
...
nsteps_before_inj = 300;
nsteps_after_inj = 400;
...
dt = [dt; rampupTimesteps((1-inj_stop)*tot_time, (1-inj_stop)*tot_time/nsteps_after_inj, 10)];
