function [masses, masses_0] = massTrappingVE_OtherNew(Gh, cVE, p, sG, sW, rock, fluidADI, varargin)
...
%}
    ...
    pv       = rock.poro(cVE) .* Gh.cells.volumes(cVE) .* pvMult; % think Gt.cells.volumes is fine -> it's just the surface area of each top surface cell
    if isfield(rock,'ntg')
       pv = rock.poro(cVE) .* Gh.cells.volumes(cVE) .* rock.ntg(cVE) .* pvMult;
