function [problem, state] = equationsWGdisgas(model, state0, state, dt, drivingForces, varargin)
%{
...
%}
   ...
   eqs(3:4) = compute_dissolution_equations(model, state, Gt, f, sG, sG0, sGmax, ...
                                            sGmax0, bG, sW, sW0, bW, bW0, p, ...
                                            rhoW, mobW, rs, rs0, rsSat, isSat, ...
                                            pvMult, pvMult0, wc, qW_pV, s, ...
                                            rsbWvW, src, dt);
...
function eqs = compute_dissolution_equations(model, state, G, f, sG, sG0, sGmax, ...
   ...
   if f.dis_rate == 0
      ...
      useDepth = true;
      [G.cells.topDepth, G.cells.bottomDepth, G.cells.height, G.cells.topCells] = getCellHeights(G, useDepth);
      rate = f.dis_rate .* s.pv./G.cells.height; % rate per area multiplied
